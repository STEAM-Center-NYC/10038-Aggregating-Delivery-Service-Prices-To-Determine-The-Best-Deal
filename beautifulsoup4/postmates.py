...
#     # .contents is used to only gather the actual text and not the code with text.
    ...
    if item_name in limit:
        ...
        # try:
            ...
            cursor.execute(f"SELECT `item_id` FROM `price` WHERE `item_id` = '{item_id}' AND `service_id` = '3'")
            dup = cursor.fetchone()
            if not dup:
                pass
            else:
            ...
            cursor.execute(f"INSERT INTO `price` (`price_value`, `item_id`, `service_id`) VALUES ('{item_price}', '{item_id}', '3')")
            connection.commit()
            cursor.close()
